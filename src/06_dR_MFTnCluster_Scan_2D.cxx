...
void _06_dR_MFTnCluster_Scan_2D() {
    ...
    std::vector<double> cluster_cuts = {2, 3, 4, 5, 6}; // Typical MFT cluster cuts
    ...
    TString firstFname = dR_dirs[0] + "/Significance_Results_MFTnCluster.root";
    ...
    gStyle->SetOptStat(0);
    TFile *outFile = new TFile("dR_MFTnCluster_Scan_2D.root", "RECREATE");
    ...
    for (const auto& pt_bin : pt_bins) {
        ...
        TH2D *h2_sig_omega = new TH2D("h2_sig_omega", Form("Significance (#omega, %s);MFTnCluster cut;dR cut", pt_title.Data()), cluster_cuts.size(), 0.5, cluster_cuts.size() + 0.5, dR_dirs.size(), 0.5, dR_dirs.size() + 0.5);
        TH2D *h2_sig_phi = new TH2D("h2_sig_phi", Form("Significance (#phi, %s);MFTnCluster cut;dR cut", pt_title.Data()), cluster_cuts.size(), 0.5, cluster_cuts.size() + 0.5, dR_dirs.size(), 0.5, dR_dirs.size() + 0.5);
        ...
        for (int j = 0; j < (int)dR_dirs.size(); ++j) {
            TString fname = dR_dirs[j] + "/Significance_Results_MFTnCluster.root";
            ...
            for (int i = 0; i < (int)cluster_cuts.size(); ++i) {
                ...
                while((tk = (TKey*)nextK())){
                    ...
                    if(kname.Contains("cluster", TString::kIgnoreCase) && kname.Contains(Form("_%.0f", cluster_cuts[i]))){
                        targetDir = kname;
                        break;
                    }
                }
                ...
                TString basePath = targetDir + "/" + pt_bin + "/Significance_";
    ...
    outFile->Close();
    std::cout << "Finish! Result saved in dR_MFTnCluster_Scan_2D.root" << std::endl;
}
