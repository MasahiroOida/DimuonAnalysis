...
void Significance_MFTnCluster(TFile *input_FitResult) {
    ...
    TFile *outfile = new TFile("Significance_Results_MFTnCluster.root", "RECREATE");
    ...
    while ((scanKey = (TKey *)scanIter())) {
        if (TString(scanKey->GetClassName()) == "TDirectoryFile") {
            ...
            if (dName.Contains("mft", TString::kIgnoreCase) || dName.Contains("cluster", TString::kIgnoreCase)) {
    ...
    std::cout << "--- Valid IDs Found (MFTnCluster) ---" << std::endl;
    ...
    while ((key = (TKey *)next())) {
        if (TString(key->GetClassName()) == "TDirectoryFile") {
            ...
            bool process = false;
            if (topName.Contains("mft", TString::kIgnoreCase) || topName.Contains("cluster", TString::kIgnoreCase)) {
                process = true; 
            ...
            while ((keyPt = (TKey *)nextPt())) {
                if (TString(keyPt->GetClassName()) == "TDirectoryFile") {
                    ...
                    for (int i = 0; i < tokens->GetEntries(); ++i) {
                        TString tok = tokens->At(i)->GetName();
                        TString tokLow = tok; tokLow.ToLower();
                        if ((tokLow == "cluster" || tokLow == "mftncluster") && i + 1 < tokens->GetEntries()) {
        ...
        auto formatLabel = [](TString dirName) -> TString {
            ...
            for (int i = 0; i < tokens->GetEntries(); ++i) {
                ...
                if ((tokLow == "cluster" || tokLow == "mftncluster") && i + 1 < tokens->GetEntries()) {
                    ...
                    label = Form("MFTnCluster >= %s", val.Data());
                    break;
                }
            }
            delete tokens;
    ...
    outfile->Close();
    std::cout << "Finish summary plots generation for MFTnCluster dependence" << std::endl;
}
