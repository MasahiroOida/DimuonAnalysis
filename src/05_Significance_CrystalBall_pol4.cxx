...
double CalcSignificanceErrorPol4(double S, double S_err, double T, double T_err, double CovST) {
...
void Significance_CrystalBall_pol4(TFile *input_FitResult) {
    ...
    TFile *outfile = new TFile("Significance_Results_pol4.root", "RECREATE");
        ...
        if (TString(key->GetClassName()) == "TDirectoryFile") {
            ...
            while ((keyPt = (TKey *)nextPt())) {
                if (TString(keyPt->GetClassName()) == "TDirectoryFile") {
                    ...
                    // Constant: 5, Mean: 6, Sigma: 7, Alpha: 8, N: 9
                    // Constant: 10, Mean: 11, Sigma: 12, Alpha: 13, N: 14
                    ...
                    double sig_o_err = CalcSignificanceErrorPol4(S_o, S_o_err, T_o, T_o_err, 0.0);
                    double sig_p_err = CalcSignificanceErrorPol4(S_p, S_p_err, T_p, T_p_err, 0.0);
    ...
    if (!summaryDataList.empty()) {
        ...
        for (auto& entry : dataByID) {
            ...
            for (const auto& d : list) if (d.ptName != "Pt_0to30") ptList.push_back(d);
    ...
    outfile->Close();
    std::cout << "Finish Significance calculation (pol4)" << std::endl;
}
