...
void Significance_MCHnCluster(TFile *input_FitResult) {
    ...
    TFile *outfile = new TFile("Significance_Results_MCHnCluster.root", "RECREATE");
    ...
    while ((scanKey = (TKey *)scanIter())) {
        if (TString(scanKey->GetClassName()) == "TDirectoryFile") {
            ...
            if (dName.Contains("mch", TString::kIgnoreCase) || dName.Contains("cluster", TString::kIgnoreCase)) {
    ...
    std::cout << "--- Valid IDs Found (MCHnCluster) ---" << std::endl;
    ...
    while ((key = (TKey *)next())) {
        if (TString(key->GetClassName()) == "TDirectoryFile") {
            ...
            bool process = false;
            if (topName.Contains("mch", TString::kIgnoreCase) || topName.Contains("cluster", TString::kIgnoreCase)) {
                process = true; 
            ...
            while ((keyPt = (TKey *)nextPt())) {
                if (TString(keyPt->GetClassName()) == "TDirectoryFile") {
                    ...
                    // Updated indices for pol4 + 2 * crystalball
                    // BG(0-4), Omega(5-9), Phi(10-14)
                    ...
                    data.ptMin = ptMin;
                    data.ptMax = ptMax;
                    data.clusterVal = 999999.0;
                    TObjArray *tokens = topName.Tokenize("_");
                    for (int i = 0; i < tokens->GetEntries(); ++i) {
                        TString tok = tokens->At(i)->GetName();
                        TString tokLow = tok; tokLow.ToLower();
                        if ((tokLow == "cluster" || tokLow == "mchncluster") && i + 1 < tokens->GetEntries()) {
                            data.clusterVal = TString(tokens->At(i+1)->GetName()).Atof();
                            break;
                        }
                    }
                    delete tokens;
    ...
    if (!summaryDataList.empty()) {
        ...
        auto formatLabel = [](TString dirName) -> TString {
            ...
            if (!dirNameLow.Contains("cluster")) return "nocut";
            ...
            for (int i = 0; i < tokens->GetEntries(); ++i) {
                ...
                TString tokLow = tok; tokLow.ToLower();
                if ((tokLow == "cluster" || tokLow == "mchncluster") && i + 1 < tokens->GetEntries()) {
                    TString val = tokens->At(i+1)->GetName();
                    label = Form("MCHnCluster >= %s", val.Data());
                    break;
                }
            }
            delete tokens;
        ...
        for (auto& entry : dataByPt) {
            ...
            std::sort(list.begin(), list.end(), [](const SummaryData& a, const SummaryData& b) {
                return a.clusterVal < b.clusterVal;
            });
            ...
            for (int i=0; i<nData; ++i) {
                ...
                h->SetMarkerSize(0.7); // マーカーサイズを小さく設定
                h->SetLineWidth(2);
                h->GetXaxis()->SetRangeUser(0.0, 1.5);
                h->GetYaxis()->SetRangeUser(0, maxVal * 1.2);
    ...
    outfile->Close();
    std::cout << "Finish summary plots generation for MCHnCluster dependence" << std::endl;
}
