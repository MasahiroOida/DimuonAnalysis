...
void _06_dR_Chi2_Scan_2D_pol4() {
    ...
    TString firstFname = dR_dirs[0] + "/Significance_Results_pol4.root";
    ...
    TFile *outFile = new TFile("dR_Chi2_Scan_2D_pol4.root", "RECREATE");
    ...
    for (const auto& pt_bin : pt_bins) {
        ...
        TH2D *h2_sig_omega = new TH2D("h2_sig_omega", Form("Significance Map (#omega, %s, pol4);#chi^{2} cut;dR cut", pt_title.Data()), chi2_cuts.size(), 0.5, chi2_cuts.size() + 0.5, dR_dirs.size(), 0.5, dR_dirs.size() + 0.5);
        TH2D *h2_sig_phi = new TH2D("h2_sig_phi", Form("Significance Map (#phi, %s, pol4);#chi^{2} cut;dR cut", pt_title.Data()), chi2_cuts.size(), 0.5, chi2_cuts.size() + 0.5, dR_dirs.size(), 0.5, dR_dirs.size() + 0.5);
        TH2D *h2_mean_omega = new TH2D("h2_mean_omega", Form("Mean Map (#omega, %s, pol4);#chi^{2} cut;dR cut", pt_title.Data()), chi2_cuts.size(), 0.5, chi2_cuts.size() + 0.5, dR_dirs.size(), 0.5, dR_dirs.size() + 0.5);
        TH2D *h2_mean_phi = new TH2D("h2_mean_phi", Form("Mean Map (#phi, %s, pol4);#chi^{2} cut;dR cut", pt_title.Data()), chi2_cuts.size(), 0.5, chi2_cuts.size() + 0.5, dR_dirs.size(), 0.5, dR_dirs.size() + 0.5);
        TH2D *h2_sigma_omega = new TH2D("h2_sigma_omega", Form("Width Map (#omega, %s, pol4);#chi^{2} cut;dR cut", pt_title.Data()), chi2_cuts.size(), 0.5, chi2_cuts.size() + 0.5, dR_dirs.size(), 0.5, dR_dirs.size() + 0.5);
        TH2D *h2_sigma_phi = new TH2D("h2_sigma_phi", Form("Width Map (#phi, %s, pol4);#chi^{2} cut;dR cut", pt_title.Data()), chi2_cuts.size(), 0.5, chi2_cuts.size() + 0.5, dR_dirs.size(), 0.5, dR_dirs.size() + 0.5);
        ...
        for (int j = 0; j < (int)dR_dirs.size(); ++j) {
            ...
            TString fname = dR_dir + "/Significance_Results_pol4.root";
            ...
            for (int i = 0; i < (int)chi2_cuts.size(); ++i) {
                ...
                if (pPtMin && pPtMax) {
                    ...
                    TParameter<double> *pPtMinAlt = (TParameter<double>*)fIn->Get(chi2DirName + "/ptmin");
                    TParameter<double> *pPtMaxAlt = (TParameter<double>*)fIn->Get(chi2DirName + "/ptmax");
                    if (pPtMinAlt && pPtMaxAlt) {
                        double pt_val = (pPtMinAlt->GetVal() + pPtMaxAlt->GetVal()) / 2.0;
                        double pt_err = (pPtMaxAlt->GetVal() - pPtMinAlt->GetVal()) / 2.0;
                        double chi2 = chi2_cuts[i];
                        TString dR = dR_dirs[j];
                        if (pMeanO)  globalPtData[chi2][dR]["mean_omega"].push_back({pt_val, pt_err, pMeanO->GetVal(), pMeanO_err ? pMeanO_err->GetVal() : 0});
                        if (pMeanP)  globalPtData[chi2][dR]["mean_phi"].push_back({pt_val, pt_err, pMeanP->GetVal(), pMeanP_err ? pMeanP_err->GetVal() : 0});
        ...
        auto DrawComparison = [&](TString name, TString title, std::vector<std::vector<double>>& vy, std::vector<std::vector<double>>& vyer) {
            TCanvas *cComp = new TCanvas("cComp_"+name+"_"+pt_bin+"_pol4", title + " (pol4)", 1000, 700);
            cComp->SetGrid();
            ...
            for (int j = 0; j < (int)dR_dirs.size(); ++j) {
                ...
                if (j == 0) {
                    gr->SetTitle(title + " vs #chi^{2} cut (" + pt_title + ", pol4);#chi^{2} cut;" + title);
                    gr->Draw("APE");
                    /*
                ...
                gr->Write(Form("gr_chi2scan_%s_%s", name.Data(), dR_dirs[j].Data()));
            }
    ...
    for (double chi2 : chi2_cuts) {
        ...
        for (const auto& var : variables) {
            TCanvas *cPt = new TCanvas(Form("c_pt_%s_chi2%.0f_pol4", var.Data(), chi2), titles[var] + " (pol4)", 1000, 700);
            cPt->SetGrid();
            TLegend *leg = new TLegend(0.7, 0.7, 0.88, 0.88);
            leg->SetHeader(Form("#chi^{2} < %.0f (pol4)", chi2));
            bool first = true;
            for (int j = 0; j < (int)dR_dirs.size(); ++j) {
                TString dR = dR_dirs[j];
                ...
                TGraphErrors *gr = new TGraphErrors(n, x.data(), y.data(), xe.data(), ye.data());
                int color = colors[j % colors.size()];
                gr->SetMarkerStyle(20); gr->SetMarkerColor(color); gr->SetLineColor(color);
                cPt->cd();
                if (first) {
                    gr->SetTitle(titles[var] + " vs p_{T} (#chi^{2} < " + Form("%.0f", chi2) + ", pol4);p_{T} (GeV/c);" + titles[var]);
                    gr->Draw("APE");
                    cPt->Update(); first = false;
                } else {
                    gr->Draw("PE SAME");
                }
                TString label = dR; label.ReplaceAll("dR_0", "dR < 0.");
                leg->AddEntry(gr, label, "lp");
                gr->Write(Form("gr_%s_%s_chi2%.0f", var.Data(), dR.Data(), chi2));
            }
            cPt->cd();
            leg->Draw(); 
            cPt->Update();
    ...
    outFile->Close();
    std::cout << "\nFinish! Result saved in dR_Chi2_Scan_2D_pol4.root with comparison plots." << std::endl;
}

void _06_dR_Chi2_Scan_2D_pol4_run() {
    _06_dR_Chi2_Scan_2D_pol4();
}
