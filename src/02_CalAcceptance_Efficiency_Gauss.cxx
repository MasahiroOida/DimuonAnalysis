...
double GetYieldFromFit(TH1D* h, TString particle, double &yieldErr, TFile* fitFile, TString nameTag) {
    ...
    if (particle.Contains("Omega", TString::kIgnoreCase)) {
        ...
        // Apply limits BEFORE setting parameters
        fitFunc->SetParLimits(1, 0.75, 0.85); // mean
        ...
        // Apply limits BEFORE setting parameters
        fitFunc->SetParLimits(1, 0.9, 1.1); // mean
        fitFunc->SetParLimits(2, 0.005, 0.05); // sigma
    ...
    if (int(r) == 0 || int(r) == 2) {
        ...
        std::cout << "Warning: Fit failed for " << nameTag << " status=" << int(r) << std::endl;
        yieldErr = 0;
        yield = 0;
    }
...
void CalAcceptance_Efficiency_Gauss() {
    ...
    TFile *fileOut = new TFile("AcceptanceEfficiency_Gauss.root", "RECREATE");
    TFile *fileFits = new TFile("AcceptanceEfficiency_Fits_Gauss.root", "RECREATE");
    ...
    fileFits->Close();
    fileIn->Close();
    std::cout << "Acceptance and Efficiency calculation finished. Canvases saved in AcceptanceEfficiency_Gauss.root." << std::endl;
}
